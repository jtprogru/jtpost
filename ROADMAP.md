...
**Версия:** 0.3.0 (UUID v7 миграция завершена)
...
- 🔴 Версия 0.3.0 в разработке (рефакторинг ID, упрощение архитектуры)
...
#### 7.1 Переход на UUID v7 ✅
- ✅ Добавлена зависимость `github.com/google/uuid v1.6.0`
- ✅ Изменён тип `PostID` с `string` на `uuid.UUID`
- ✅ Обновлена `GeneratePostID()` для генерации UUID v7
- ✅ Обновлены методы `Value()/Scan()` для SQLite (конвертация в строку)
- ✅ Обновлены все репозитории (FS, SQLite)
- ✅ Обновлены CLI команды для парсинга UUID
- ✅ Обновлены HTTP API обработчики
- ✅ Обновлены все тесты
- ✅ Написана команда миграции `jtpost migrate-ids`

#### 7.3 Удаление упоминаний платформ ✅
- ✅ Удалён тип `Platform` из `internal/core/core.go`
- ✅ Удалено поле `Platforms` из структуры `Post`
- ✅ Удалён фильтр по платформам из `PostFilter`
- ✅ Удалён флаг `--platform` из CLI команд
- ✅ Удалён endpoint `/api/platforms`
- ✅ Удалён селектор платформ из Web UI
- ✅ Удалено поле `platforms` из SQLite схемы
- ✅ Обновлена команда `publish` (убран флаг `--to`)
  ```bash
  # Было:
...
#### 7.2 Отображение идентификаторов ✅
- [x] Добавить колонку "ID" в `jtpost list` (показана по умолчанию)
- [x] Опциональный флаг `--no-id` для скрытия ID
- [x] Добавить колонку ID в Web UI (короткий префикс UUID + tooltip с полным ID)
- [x] Добавить отображение ID в модальном окне (readonly + кнопка «📋 Копировать»)
- [x] Добавить кнопку "Копировать ID" (копирование по клику в таблице и из модалки)

#### 7.4 Чистка Web UI ✅
- [x] Удалить раздел "Публикация в Telegram" из модального окна (сделано в 0.3.0)
- [x] Удалить мёртвый код `openPublishModal()` / `publish-status` в `closePostModal`
- [x] Оставить кнопку "📨 Опубликовать" в таблице постов
- [x] Подтверждение публикации через нативный `confirm()` (отдельная кастомная модалка отложена — текущий UX достаточен)
...
### Этап 8: Улучшение Telegram Publisher (отложено) 🔴
...
### Этап 13: Документация и примеры 🟡

**Статус:** 🟡 Частично реализовано
**Приоритет:** Средний

**Задачи:**
- [x] Примеры использования в `examples/` (config + sample post + README)
- [ ] Видео-туториалы
- [x] API документация (OpenAPI 3.1 spec — этап B.3)
- [x] Генерация документации CLI (`jtpost docs` → `docs/cli/`)
- [x] Перевод документации на английский (README.en.md, основной README)

---

## 📅 Календарный план

### Март 2026 — Версия 0.3.0 ✅

**Неделя 1-2:**
- ✅ Рефакторинг идентификаторов (UUID v7)
- ✅ Удаление упоминаний платформ
...
**Результат:** Версия 0.3.0 с улучшенной архитектурой и UUID v7 ✅

---

### Апрель 2026 — Версия 0.4.0 🔴

**Неделя 1-2:**
- 🔴 Git Repository хранилище
- 🔴 Улучшение Telegram Publisher (опционально)

**Неделя 3-4:**
- 🔴 PostgreSQL хранилище
- 🔴 CI/CD настройка

**Результат:** Версия 0.4.0 с Git и PostgreSQL хранилищами

---

### Май 2026 — Версия 0.5.0 🔴
...
**Результат:** Версия 0.5.0 с улучшенным UI и документацией
...
- [x] **Производительность:** Benchmark тесты для критичных участков (DiffLines, frontmatter parse/serialize, telegramconv)
...
- 🔴 Рефакторинг идентификаторов (UUID v7) — в работе
- 🔴 Упрощение архитектуры (удаление платформ) — в работе
...
### 2026-03-19 — Версия 0.3.0 ✅

**Завершено:**
- ✅ Рефакторинг идентификаторов (переход на UUID v7)
- ✅ Удаление упоминаний платформ (фокус только на Telegram)

**Добавлено:**
- ✅ Зависимость `github.com/google/uuid v1.6.0`
- ✅ Команда `jtpost migrate-ids` для миграции старых ID

**Изменено:**
- ✅ Тип `PostID` изменён с `string` на `uuid.UUID`
- ✅ Обновлена команда `publish`: `jtpost publish <id>` (без флага `--to`)
- ✅ Удалён тип `Platform` и поле `Platforms` из структур данных
- ✅ Удалён endpoint `/api/platforms`
- ✅ Обновлены все репозитории (FS, SQLite) для работы с UUID
- ✅ Обновлены CLI команды и HTTP API обработчики

**Исправлено:**
- ✅ Все тесты обновлены для работы с UUID v7
- ✅ Линтер чист (0 issues)

**Документация:**
- ✅ Обновлён ROADMAP.md
- ✅ Обновлён README.md
- ✅ Обновлены AGENTS.md и QWEN.md
