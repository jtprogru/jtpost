...
- ✅ Хранилища: filesystem, SQLite, Git (auto-commit), PostgreSQL
- ✅ Миграция между хранилищами (`jtpost migrate --from --to`)
- ✅ HTTP API + Web UI v2 (htmx + templ, dark/light, PWA)
- ✅ Multi-tenancy + RBAC (users, PAT-токены, sessions, OAuth GitHub)
- ✅ Audit log, structured logging, rate limiting
- ✅ Worker/outbox с exponential backoff и recovery sweep
- ✅ History/diff/revert постов через git-storage
- ✅ UUID v7 идентификаторы
...
Скачайте готовый бинарник со страницы [Releases](https://github.com/jtprogru/jtpost/releases). Подпись `checksums.txt` GPG-ключом проверяется через `gpg --verify checksums.txt.sig checksums.txt`.
...
| **Хранилища** | 4 (FS, SQLite, Git, Postgres) |
| **Платформы** | 1 (Telegram, с media groups) |
| **Статусы поста** | 5 |
| **Формат ID** | UUID v7 |
| **Тесты** | 100% PASS |
| **Линтер** | 0 issues |
